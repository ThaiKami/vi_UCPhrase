import utils
import string
from consts import consts as consts_class
import torch
...
class BaseModel(nn.Module):
    ...
    def from_config(cls, config_or_path_or_dir):
        ...
        if isinstance(config_or_path_or_dir, dict):
            ...
            assert isinstance(config_or_path_or_dir, str) or isinstance(
                config_or_path_or_dir, Path
            )
    ...
    def _par_decode_doc(predicted_doc, threshold):
        ...
        for predicted_sent in predicted_doc["sents"]:
            tokens = c.LM_TOKENIZER.convert_ids_to_tokens(predicted_sent["ids"])
            ...
            for l_idx, r_idx, prob in predicted_spans:
                ...
                if idxs_set & idxs_taken:
                    continue
                idxs_taken |= idxs_set
                phrase = roberta_tokens_to_str(tokens[l_idx : r_idx + 1])
    ...
    def decode(path_predicted_docs, output_dir, threshold, use_cache, use_tqdm):
        ...
        path_output = output_dir / (
            f"doc2sents-{threshold}-" + path_predicted_docs.stem + ".json"
        )
        ...
        to_iterate = (
            tqdm(predicted_docs, ncols=100, desc="Decode")
            if use_tqdm
            else predicted_docs
        )

        doc2sents = {
            doc["_id_"]: BaseModel._par_decode_doc(doc, threshold) for doc in to_iterate
        }
    ...
    def _par_get_doc_cands(predicted_doc, threshold, filter_punc=True):
        ...
        for predicted_sent in predicted_doc["sents"]:
            ...
            for l_idx, r_idx, prob in predicted_spans:
                if prob > threshold:
                    cand = phobert_tokens_to_str(tokens[l_idx : r_idx + 1])
    ...
    @staticmethod
    def get_doc2cands(
        path_predicted_docs, output_dir, expected_num_cands_per_doc, use_cache, use_tqdm
    ):
        output_dir = Path(output_dir)
        path_output = output_dir / (
            f"doc2cands-{expected_num_cands_per_doc}-"
            + path_predicted_docs.stem
            + ".json"
        )
        if use_cache and utils.IO.is_valid_file(path_output):
        ...
        for _ in range(20):
            ...
            doc2cands = {
                doc["_id_"]: BaseModel._par_get_doc_cands(doc, threshold)
                for doc in to_iterate
            }

            num_cands = [
                len(cands)
                for doc, cands in doc2cands.items()
                if doc in c.DOCIDS_WITH_GOLD
            ]
...
class DecodedCorpus:
    ...
    def dump_html(self):
        path_output = self.path_decoded_doc2sents.with_name(
            self.path_decoded_doc2sents.name.replace("doc2sents", "html")
        ).with_suffix(".html")
        html_lines = []
        for doc, sents in self.decoded_doc2sents.items():
            ...
            for sent in sents:
                ...
                for l, r, _ in sent["spans"]:
                    tokens[l] = c.HTML_BP + tokens[l]
                    tokens[r] = tokens[r] + c.HTML_EP + " |"
                html_lines.append(roberta_tokens_to_str(tokens))
    ...
    def _batchify(marked_sents, is_train=False):
        ...
        while pointer < total_num:
            ...
            for marked_sent in marked_sents[pointer : pointer + batch_size]:
                ...
                if not is_train:
                    possible_spans = utils.get_possible_spans(
                        word_idxs, len(input_id), c.MAX_WORD_GRAM, c.MAX_SUBWORD_GRAM
                    )
                ...
                input_ids_batch.append(
                    [c.LM_TOKENIZER.bos_token_id]
                    + input_id
                    + [c.LM_TOKENIZER.pad_token_id] * len_diff
                )
            ...
            pointer += batch_size
            input_ids_batch = torch.tensor(input_ids_batch, device=c.DEVICE)
            input_masks_batch = torch.tensor(input_masks_batch, device=c.DEVICE)
            if is_train:
                batches.append(
                    (
                        input_ids_batch,
