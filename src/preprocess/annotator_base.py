...
class BaseAnnotator:
    def __init__(self, preprocessor: Preprocessor, use_cache=True):
        ...
        self.dir_output = (
            self.preprocessor.dir_preprocess / f"annotate.{self.__class__.__name__}"
        )
    ...
    def _par_sample_train_data(marked_doc):
        ...
        for sent in sents:
            ...
            all_spans = utils.get_possible_spans(
                word_idxs,
                len(sent["ids"]),
                c.MAX_WORD_GRAM,
                c.MAX_SUBWORD_GRAM,
            ...
            num_negative = min(
                len(possible_negative_spans), int(num_positive * c.NEGATIVE_RATIO)
            )
            sampled_negative_spans = random.sample(
                possible_negative_spans, k=num_negative
            )
    ...
    def sample_train_data(self):
        ...
        path_output = (
            self.dir_output
            / f"sampled.neg{c.NEGATIVE_RATIO}.{self.path_marked_corpus.name}"
        )
        ...
        sampled_docs = [
            ...
            for d in tqdm(marked_docs, ncols=100, desc="[Sample Train]")
        ]
    ...
    def get_path_sampled_train_spans(path_sampled_train_data):
        ...
        path_output = path_sampled_train_data.with_name(
            f"{path_sampled_train_data.stem}.spans.json"
        )
        ...
        spans = []
        for marked_sent in tqdm(
            marked_sents, ncols=100, desc="[Annotator] get positive spans"
        ):
        ...
        spans = [
            (
                label,
                "".join(c.PRETRAINED_TOKENIZER.convert_ids_to_tokens(ids))
                .replace(c.GPT_TOKEN, " ")
                .strip(),
            )
        ...
        for raw_id_doc in marked_corpus:
            for sent in raw_id_doc["sents"]:
                sent["phrases"] = [
                    ...
                    if p[0][1] - p[0][0] + 1 <= c.MAX_SUBWORD_GRAM
                ]
            # print(f"""marked_corpus {raw_id_doc}""")
