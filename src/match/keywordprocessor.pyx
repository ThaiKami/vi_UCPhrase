""" cython version of FlashText.keywordprocessor
"""
...
cdef class KeywordProcessor(object):
    cdef char* _keyword
    cdef set _white_space_chars, non_word_boundaries
    cdef dict keyword_trie_dict
    cdef bint case_sensitive
    cdef int _terms_in_trie

    def __init__(self, case_sensitive=False, use_bpe_tokenzier=False):
        """
        Args:
        ...
        try:
            # python 2.x
            self.non_word_boundaries = set(string.digits + string.letters + string.punctuation) | {'Ġ', 'Ġ'.lower()}
        except AttributeError:
            # python 3.x
            self.non_word_boundaries = set(string.digits + string.ascii_letters + string.punctuation) | {'Ġ', 'Ġ'.lower()}
        self.keyword_trie_dict = dict()
        self.case_sensitive = case_sensitive
        self._terms_in_trie = 0
        self.remove_non_word_boundaries('"')
    ...
    def __setitem__(self, keyword, clean_name):
    ...
    def get_non_word_boundary(self):
        return self.non_word_boundaries

    def remove_non_word_boundaries(self, chars):
        self.non_word_boundaries -= set(chars)
    ...
    cpdef str replace_keywords(self, str sentence):
        """Searches in the string for all keywords present in corpus.
        ...
        cdef list new_sentence
        cdef str orig_sentence, current_word, current_word_continued, current_white_space, sequence_found, longest_sequence_found
        cdef int sequence_end_pos, idx, sentence_len, last_idx, pbar_threshold, pbar_gap
        cdef bint is_longer_seq_found
        cdef str c
        ...
        last_idx = 0
        sentence_len = len(sentence)
        pbar_gap = int(sentence_len / 100)
        pbar_threshold = pbar_gap
        pbar = tqdm(total=sentence_len, ncols=100, desc='Replace keywords')
        while idx < sentence_len:
            if idx > pbar_threshold:
                pbar.update(idx - last_idx)
                pbar_threshold += pbar_gap
                last_idx = idx
            c = sentence[idx]
            current_word += orig_sentence[idx]
            # when we reach whitespace
            if c not in self.non_word_boundaries:
                current_white_space = c
                # if end is present in current_dict
                if self._keyword in current_dict or c in current_dict:
                    # update longest sequence found
                    sequence_found = None
                    longest_sequence_found = None
                    is_longer_seq_found = False
                    ...
                    if c in current_dict:
                        current_dict_continued = current_dict[c]
...
if __name__ == '__main__':
    ...
    with open('../../data/kp20k/tokenized_train.txt') as rf:
        text = rf.read()
    with open('../../data/kp20k/wiki_quality.txt') as rf:
        phrases = rf.read().splitlines()
    phrase_dict = {p.replace(' ', '_'): [p] for p in phrases}
    processor.add_keywords_from_dict(phrase_dict)
