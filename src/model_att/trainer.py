...
class AttmapTrainLoader:
    ...
    def get_loader(self, instances, is_train=True):
        ...
        instances = [
            ...
            if instance[2].shape[-1] <= self.max_num_subwords
        ]
        ...
        attmap_features = AttmapModel.pad_attention_maps(
            attention_maps, max_num_subwords=self.max_num_subwords
        )
    ...
    def load_train_data(self, filepath, sample_ratio=-1):
        ...
        train_instances, valid_instances = train_test_split(
            instances, test_size=0.1, shuffle=True, random_state=self.random_seed
        )
...
class AttmapTrainer:
    ...
    def train(self, path_sampled_train_data, num_epochs=20):
        ...
        path_train_data = self.model.feature_extractor.generate_train_instances(
        ...
        train_data, valid_data = self.train_loader.load_train_data(
            path_train_data, sample_ratio=self.sample_ratio
        )
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            self.model.train()
            epoch_loss = 0
            for attmap_features, gtlabels in tqdm(
                train_data, total=num_train, ncols=100
            ):
            ...
            with torch.no_grad():
                for attmap_features, gtlabels in tqdm(
                    valid_data, total=num_valid, ncols=100
                ):
                    attmap_features = attmap_features.to(DEVICE)
