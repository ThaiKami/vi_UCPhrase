...
class FeatureExtractor(BaseFeatureExtractor):
    def __init__(self, output_dir, num_BERT_layers, use_cache=True):
    ...
    def _get_model_outputs(self, marked_sents):
        ...
        with torch.no_grad():
            for input_ids_batch, input_masks_batch, _ in tqdm(
                input_idmasks_spans_batches, ncols=100, desc="inference"
            ):
                model_output = consts.LM_MODEL(
                    input_ids_batch,
                ...
                )  # layers, [batch_size, num_heads, seqlen, seqlen]
                batch_attentions = torch.stack(
                    batch_attentions, dim=0
                )  # [layers, batch_size, num_heads, seqlen, seqlen]
                batch_size, seq_len = input_ids_batch.shape
                batch_attentions = batch_attentions.transpose(0, 1)[
                    :, : self.num_BERT_layers, :, 1:, 1:
                ]
    ...
    def generate_train_instances(self, path_sampled_docs, max_num_docs=None):
        ...
        path_output = (
            self.output_dir / path_sampled_docs.name.replace("sampled.", path_prefix)
        ).with_suffix(".pk")
        ...
        sampled_docs = (
            sampled_docs[:max_num_docs] if max_num_docs is not None else sampled_docs
        )
        print("OK!")
        ...
        for i, model_output_dict in tqdm(
            ...
            for negative_l_idx, negative_r_idx in marked_sent["neg_spans"]:
                negative_wl_idx, negative_wr_idx = (
                    swidx2widx[negative_l_idx],
                    swidx2widx[negative_r_idx + 1] - 1,
                )
                negative_spanlen = negative_wr_idx - negative_wl_idx + 1
                negative_span_attentionmap = self._get_span_attenionmap(
                    model_output_dict, negative_l_idx, negative_r_idx
                )
                negative_instance = (
                    0,
                    negative_spanlen,
                    negative_span_attentionmap,
                    marked_sent["ids"][negative_l_idx : negative_r_idx + 1],
                )
    ...
    def generate_predict_docs(self, path_marked_corpus, max_num_docs=None):
        ...
        test_feature_dir = (
            ...
            / "LM_output_for_prediction"
            / f"Attmap.{consts.LM_NAME_SUFFIX}.{self.num_BERT_layers}layers"
        )
        test_feature_dir.mkdir(exist_ok=True, parents=True)

        predict_name = "predict.batch.float16." + f"{max_num_docs}docs." * (
            max_num_docs is not None
        )
        path_output = (
            test_feature_dir / path_marked_corpus.name.replace("marked.", predict_name)
        ).with_suffix(".pk")
        print(path_output)
        if self.use_cache and utils.IO.is_valid_file(path_output):
        ...
        marked_docs = (
            marked_docs[:max_num_docs] if max_num_docs is not None else marked_docs
        )
        ...
        for i, model_output_dict in tqdm(
            ...
            ncols=100,
            total=len(marked_sents),
            desc="Generate predict instances",
        ):
            marked_sent = marked_sents[i]
            word_idxs = marked_sent["widxs"]
            ...
            del model_output_dict
        predict_instances = [
            predict_instances[rawidx2newidx[rawidx]]
            for rawidx in range(len(predict_instances))
        ]
